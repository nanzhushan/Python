...
class Example3(wx.Frame):   #继承父类
    def __init__(self):
        wx.Frame.__init__(self, parent=None, id=-1, title=u'笑傲江湖', size=(600, 600))
        ...
        #基于BoxSizer布局
        # hBoxAll = wx.BoxSizer(wx.HORIZONTAL)   #这个盒子装了其他盒子
        # vBoxControl = wx.BoxSizer(wx.VERTICAL)   #这个盒子装控制部分的文件
        # hBoxCommand = wx.BoxSizer(wx.HORIZONTAL)    #盒子 装配置命令的部分，包括静态文本和文本输出框
        # vBoxWrite = wx.BoxSizer(wx.VERTICAL)      #装python代码的
        # hBoxButton = wx.BoxSizer(wx.HORIZONTAL)   #按钮盒子装所有按钮
