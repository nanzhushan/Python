...
class Example3(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, parent=None, id=-1, title=u'龙行天下', size=(600, 600))
        self.panel = wx.Panel(self, -1)
        self.Centre()

        #定义我们需要的各个控件

        commandStatic = wx.StaticText(self.panel, -1, u'输命令:')   #静态文本
        ...
        self.serverList = ['192.168.0.4', '223.5.5.5','192.168.6.11', '202.96.134.133']
...
#定义事件函数
    def _onSend(self, event):
        ...
        print u'写入的Python代码是:\n%s' % writePy    #把执行的python代码也打印出来

    def onClear(self, event):
        self.commandText.Clear()   #把 commandText文本框清空
